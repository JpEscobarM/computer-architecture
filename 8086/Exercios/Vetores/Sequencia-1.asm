Exercícios Sequências - Solução 1
; Programa que lê e escreve um vetor de números inteiros
; de 8 bits sem sinal utilizando endereçamento indireto
...
.data 
    ...
    LF EQU 10           ; define uma constante de valor 10
     ...
     vetor8 db TAM_VETOR dup(?)   ; 5 bytes da memoria nao inicializados
.code  
...
INICIO:  
    ...
    mov BX, offset vetor8   ; NUNCA utilizem LEA lea BX, VETOR
    mov CX, 5   
                   
    LACO_PRINCIPAL:                         
       call LER_UINT16
       mov [BX], AL   
       inc BX      ; Como é somente 1 byte, deve-se incrementar de uma unidade
       loop LACO_PRINCIPAL 
    ...
    ESCREVE_VETOR:
        mov AL, [SI]
        call ESC_UINT16 
        inc SI          ; Como é somente 1 byte, deve-se incrementar de uma unidade
        mov DL, CR      ; Dar um enter após a leitura          
        call ESC_CHAR
        mov DL, LF             
        call ESC_CHAR
